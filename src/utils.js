...
export const KNOWLEDGE_GAPS_CONFIG = {
  LOW_RELEVANCE_THRESHOLD: 20,  // Score below this = knowledge gap
  MAX_GAPS: 50,                 // Maximum gaps to store
  MIN_QUERY_LENGTH: 10,         // Don't track very short queries
};
...
export const DUPLICATE_CONFIG = {
  ...
  TITLE_WEIGHT: 0.4,           // Title contributes 40% to similarity
  CONTENT_WEIGHT: 0.6,         // Content contributes 60%
  COMPARE_CHARS: 300,          // Compare first N chars of content
};
...
export const checkForDuplicate = (newItem, intelligenceIndex) => {
  ...
  for (const existing of intelligenceIndex) {
    // Only compare same source types and departments
    ...
    // Calculate content similarity (first N chars)
...
export const recordKnowledgeGap = (query, topScore, department) => {
  ...
  if (existingIndex >= 0) {
    // Update existing gap - increment count
...
// ============ CONVERSATION MEMORY (Item 7) ============
export const CONVERSATION_MEMORY_CONFIG = {
  MAX_MESSAGES_PER_DEPT: 20,
  SUMMARY_TRIGGER: 10,  // Generate summary after this many messages
};

export const getConversationSummary = (messages) => {
  if (!messages || messages.length === 0) return null;
  
  // Get last few user messages to summarize context
